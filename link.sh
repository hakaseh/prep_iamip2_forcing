...
#EC-Earth3 ssp585: missing prsn, psl
...
do
    ...
    #EC-Earth3 is given on regular grid (gr), except for friver (gn).
    elif [ $model = 'EC-Earth3' ]
    then
        if [ ${vlist[it2]} = 'friver' ]
        then
            ln -s ${moddir}/${flist[it2]}/${vlist[it2]}/gn/v20200918/${vlist[it2]}_${flist[it2]}_${model}_${exp}_${variant}_gn_*.nc $outdir
        else
